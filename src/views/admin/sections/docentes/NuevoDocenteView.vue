<template>
	<div id="nuevoProf" class="carousel slide"  data-bs-touch="false" data-bs-ride="carousel">
		<div class="carousel-inner">
			<div class="carousel-item active" id='datospersonales'>
				...
				<div class='row'>
					<div class='col-12 col-md-6' style='padding-bottom:2vw'>
						...
						<input type='text' class='form-control' v-model='form.nombre' tabindex=1 required>
					</div>
					<div class='col-12 col-md-6' style='padding-bottom:2vw'>
						<label>Apellido(s)</label>
						<input type='text' class='form-control' v-model='form.apellido'  tabindex=2 required>
					</div>
				</div>
				<div class='row'>
					<!--<div class='col-12 col-md-6' style='padding-bottom:2vw'>
						<label>Sexo</label>
						<select id='genero' class='form-control'  v-on:change='updData()' required>
					...
					<div class='col-12 col-md-6' style='padding-bottom:2vw'>
						...
						<div class='input-group'>
							<select v-model="form.month" @change="setDays()" class='form-control' required>
							...
							<select v-model="form.day" @change="setDays()" class='form-control' required>
								<option selected disabled>Dia</option>
								<option v-for='i in dia' :value=i>{{i}}</option>
							</select>
							<select v-model="form.year" @change="setDays()" class='form-control' required>
								<option selected disabled>Año</option>
								<option v-for='i in anio' :value=i>{{i}}</option>
							</select>
						</div>
					</div>
					<div class='col-12 col-md-6' style='padding-bottom:2vw'>
						<label>Nacido en</label>
						<select v-model="form.pais" tabindex=4 class='form-control' required>
				...
				<div class='row'>
					<div class='col-12 col-md-6' style='padding-bottom:2vw'>
						<label for='tipodoc'>Tipo y Nº de Documento</label>		
						<div class='input-group'>	
							<select v-model='form.tipodoc' class='form-control' required>
								<option disabled>Seleccione...</option>
								<option value='1' selected>DNI Tarjeta</option>
								<option value='2'>DNI Libreta</option>
								<option value='3'>LE/LC</option>
								<option value='4'>Pasaporte</option>
								<option value='5'>Otro</option>
							</select>
							<input type='text' class='form-control' style='margin-left:10px' v-model='form.nrodoc'  tabindex=5 placeholder='Ingrese sólo números' required>
						</div>
					</div>
					<div class='col-12 col-md-6' style='padding-bottom:2vw'>
						<label for='nrocuil'>Nro de CUIT/CUIL</label>
						<div class='input-group'>
							<input type='text' class='form-control' v-model='form.nrocuil' tabindex=6 placeholder='XX-XXXXXXXX-X' required>
						</div>
					</div>
				</div>
				<div class='row'>
					<div class='col-12' style='padding-bottom:2vw'>
						...
						<input type='text' class='form-control' required v-model='form.direccion' placeholder='Calle/Altura/Piso/Depto'>
					</div>
					<div class='col-12 col-lg-6' style='padding-bottom:2vw'>
						<label>Provincia:</label>
						<select @change='updLocalidad()' v-model='form.provincia' class='form-control' required>
							<option selected disabled>Seleccione</option>
							<option v-for='item in arrProv' :value='item.codigo'>{{item.nombre}}</option>
						</select>
					</div>
					<div class='col-12 col-lg-6' style='padding-bottom:2vw'>
						<label>Localidad:</label>
						<select v-model='form.localidad' class='form-control' required>
							<option selected disabled>Seleccione</option>
							<option v-for='item in arrLocalidad' :value='item.codigo'>{{item.nombre}}</option>
						</select>
					</div>
				</div>
				<div class='row'>
					<div class='col-6'><input value='Atrás' type='button' class='btn btn-primary' @click='moveSlide()'></div>
					<div class='col-6'><input value='Continuar' type='button' class='btn' @click='checkData()'></div>
				</div>
			</div>
			<div class='carousel-item' id='datoscontacto'>
				...
				<div class='row'>
					<div class='col-12 col-md-6' style='padding-bottom:2vw'>
						...
						<input type='text' v-model='form.telfijo' class='form-control'  tabindex=13>
					</div>
					<div class='col-12 col-md-6' style='padding-bottom:2vw'>
						<label>Teléfono Celular</label>
						<input type='text' v-model='form.telcelular' class='form-control'  tabindex=14>
					</div>
				</div>
				<div class='row'>
					<div class='col-12 col-md-6' style='padding-bottom:2vw'>
						<label>Dirección de e-mail</label>
						<input type='text' v-model='form.email' tabindex=17 class='form-control' placeholder='Ingrese su e-mail' required>
					</div>
					<div class='col-12 col-md-6' style='padding-bottom:2vw'>
						<label>Re-ingresar e-mail</label>
						<input type='text' v-model='form.email2' tabindex=18 class='form-control' placeholder='Reingrese su e-mail' required>
					</div>
				</div>
				<div class='row'>
					<div class='col-6'><input value='Atrás' class='btn btn-primary' type='button' @click='moveSlide()'></div>
					<div class='col-6'><input value='Continuar' class='btn' type='button' @click='checkData(1)'></div>
				</div>
			</div>
			<div class='carousel-item' id='estudiosprevios'>
				<div class='row' style='margin-top:20px;border-top:1px solid white;text-align:center'>
					<h4>Información docente</h4>
				</div>
				<div class='row'>
					<div class='col-12 col-md-4' style='padding-bottom:2vw'>
						<label>Fecha de ingreso a la docencia</label>
						<input type='date' required v-model='form.fechaDocencia' class='form-control'>
					</div>
					<div class='col-12 col-md-4' style='padding-bottom:2vw'>
						<label>Fecha de ingreso a la rama</label>
						<input type='date' required v-model='form.fechaRama' class='form-control'>
					</div>
					<div class='col-12 col-md-4' style='padding-bottom:2vw'>
						<label>Fecha de ingreso a la Institución</label>
						<input type='date' required v-model='form.fechaInstitucion' class='form-control'>
					</div>
				</div>
				<div class='row'>
					<div class='col-6'><input value='Atrás' class='btn btn-primary' type='button' onclick='moveSlide()'></div>
					<div class='col-6'><input value='Guardar' class='btn btn-danger' type='button' v-on:click='checkData(2)'></div>
				</div>
			</div>
		</div>
	</div>
...
const setDays = function() {
	let fecha = [form.value.day, form.value.month, form.value.year]
	let numDays = {
		'1': 31, '2': 28, '3': 31, '4': 30, '5': 31, '6': 30,
		'7': 31, '8': 31, '9': 30, '10': 31, '11': 30, '12': 31
	}
	let nDays, oDaysSelLgth
	nDays = numDays[fecha[1].value]
	if (nDays == 28 && fecha[2].value % 4 == 0)	++nDays
	oDaysSelLgth = fecha[0].length
	if (nDays != oDaysSelLgth){
		this.dia = []
		for (i=1;i<=nDays;i++) this.dia.push(i)
	}
	form.value.fNac = fecha[2].value + '-' + fecha[1].value + '-' + fecha[0].value
}

const moveSlide = (s=false) => {
	if (s) this.carousel.next()
	else this.carousel.prev()
},
const checkRequired = (obj) => {
	let objElem = obj.querySelectorAll("[required]")
	let checkedData = true
	for (var i = 0; i < objElem.length; i++) {
		if (objElem[i].value == "" || (objElem[i].nodeName == "SELECT" && objElem[i].selectedIndex == 0)) {
			objElem[i].classList.add("invalido")
			checkedData = false
		}
		else objElem[i].classList.remove("invalido")
	}
	return checkedData
}
...
const checkData = (i=0) => {
	...
	switch(i){
		case 0:
			chk = this.checkRequired(document.getElementById("datospersonales"));
			if (chk) this.moveSlide(true);
			else globalMsg("Complete correctamente todos los campos requeridos", true);
		break;
		case 1:
			chk = this.checkRequired(document.getElementById("datoscontacto"));
			if (!chk) globalMsg("Complete los campos requeridos", true);
			else if (this.datosIngresante.email != this.datosIngresante.email2) globalMsg("Las direcciones de e-mail no coinciden", true);
			else this.moveSlide(true);
		break;
		case 2:
			chk = this.checkRequired(document.getElementById("estudiosprevios"));
			if (!chk) globalMsg("Complete todos los campos requeridos", true);
			else this.postData(event.target);
		break;
	}
}
...
const updLocalidad  = async () => {
	this.updData();
	this.datosIngresante.localidad = "";
	document.getElementById("localidad").selectedIndex = 0;
	let d = {
		codProvincia:event.target.value,
	};
	apiData.setData("docs",2,d);
	this.arrLocalidad = [];
	api.get().then(r=>r.payload.forEach(value=>this.arrLocalidad.push(value)));
}
...
mounted:function(){
	...
	this.dia= [];
	this.anio = [];
	for (i = 1930; i < anio; i++) this.anio.push(i);
	for (i = 1; i < 32; i++) this.dia.push(i);
	let slides = document.querySelector("#nuevoProf");
	this.carousel = new bootstrap.Carousel(slides, {ride:false});
	apiData.setData("docs",1);
	api.get().then(r=>r.payload.forEach(value=>this.arrProv.push(value)));
}
