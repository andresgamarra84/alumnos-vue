...
function resolve(result) {
  emit('resolve', {
    ok: result,
    value: props.type === 2 && result ? inputValue.value : null
  })
  inputValue.value = ''
}
    ...
    >
      ...
      <div class="modal-buttons text-end">
        <template v-if="type === 1">
          <button class="btn btn-success me-2" @click="resolve(true)">Sí</button>
          <button class="btn btn-danger" @click="resolve(false)">No</button>
        </template>

        <template v-else>
          <button class="btn btn-primary" @click="resolve(true)">
            Aceptar
          </button>
        </template>
      </div>
